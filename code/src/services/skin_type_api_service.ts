...
class Skin_typeApiService {
	// préfixe de l'API
	private prefix = "/api/skin_type";

	// sélection de tous les enregistrements
	public selectAll = async (): Promise<ApiResponse<Skin_type[]>> => {
	...
	public selectOne = async (id: number): Promise<ApiResponse<Skin_type>> => {
		// configurer la requête HTTP
		const request = new Request(
	...
	public insert = async (data: FormData): Promise<ApiResponse<Skin_type>> => {
		// configurer la requête HTTP
		const request = new Request(
			`${import.meta.env.VITE_API_URL}${this.prefix}`,
			{
...
export default Skin_typeApiService;
