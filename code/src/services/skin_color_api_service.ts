...
class Skin_colorApiService {
	// préfixe de l'API
	private prefix = "/api/skin_color";

	// sélection de tous les enregistrements
	public selectAll = async (): Promise<ApiResponse<Skin_color[]>> => {
	...
	public selectOne = async (id: number): Promise<ApiResponse<Skin_color>> => {
		// configurer la requête HTTP
		const request = new Request(
	...
	public insert = async (data: FormData): Promise<ApiResponse<Skin_color>> => {
		// configurer la requête HTTP
		const request = new Request(
			`${import.meta.env.VITE_API_URL}${this.prefix}`,
			{
...
export default Skin_colorApiService;
